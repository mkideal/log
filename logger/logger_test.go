...
func TestLevel(t *testing.T) {
	...
	assert.Equal(t, TRACE, MustParseLevel("4"))
	assert.Equal(t, DEBUG, MustParseLevel("3"))
	assert.Equal(t, INFO, MustParseLevel("2"))
	assert.Equal(t, WARN, MustParseLevel("1"))
	assert.Equal(t, ERROR, MustParseLevel("0"))
...
func (p *mockProvider) Close() { p.Wait() }
